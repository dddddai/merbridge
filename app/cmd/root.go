...
func init() {
	...
	_ = rootCmd.PersistentFlags().MarkDeprecated("kind", "this flag is deprecated")
	...
	rootCmd.PersistentFlags().BoolVar(&config.WatchLocalPods, "watch-local-pods", true, "If enabled, Merbridge will only watch pods on the current node, only works when CNI plugin is enabled.")
