...
static inline __u32 get_curr_pod_ip(struct bpf_sock_addr *ctx)
{
...
static inline int from_app(struct bpf_sock_addr *ctx, __u32 curr_pod_ip)
{
    ...
    __u64 cookie = bpf_get_socket_cookie_addr(ctx);
    // app call others
    debugf("call from user container: cookie: %d, ip: 0x%x, port: %d", cookie,
           ctx->user_ip4, bpf_htons(ctx->user_port));
    ...
    if (curr_pod_ip) {
        ...
        if (pod) {
            int exclude = 0;
            IS_EXCLUDE_PORT(pod->exclude_out_ports, ctx->user_port, &exclude);
            if (exclude) {
            ...
            int include = 0;
            IS_INCLUDE_PORT(pod->include_out_ports, ctx->user_port, &include);
            if (!include) {
            ...
            IS_INCLUDE_IPRANGES(pod->include_out_ranges, ctx->user_ip4,
                                &include);
            if (!include) {
                debugf("dest %x not in pod(%x)'s include_out_ranges, ignored.",
                       ctx->user_ip4, ip);
                return 1;
            }
        } else {
...
static inline int to_app(struct bpf_sock_addr *ctx, struct pod_config *pod)
{
    __u32 ip = ctx->user_ip4;
    int exclude = 0;
    IS_EXCLUDE_PORT(pod->exclude_in_ports, ctx->user_port, &exclude);
    if (exclude) {
        debugf("ignored dest port by exclude_in_ports, ip: %x, port: %d", ip,
    ...
    if (!include) {
        debugf("ignored dest port by include_in_ports, ip: %x, port: %d", ip,
               bpf_htons(ctx->user_port));
        return 1;
    }
    if (bpf_htons(ctx->user_port) == pod->status_port) {
        debugf("ignored dest port by pod->status_port, ip: %x, port: %d", ip,
               bpf_htons(ctx->user_port));
        return 1;
    }
    ...
    __u64 cookie = bpf_get_socket_cookie_addr(ctx);
    bpf_map_update_elem(&cookie_original_dst, &cookie, &origin, BPF_ANY);
    // redirect it to envoy
    ctx->user_port = bpf_htons(IN_REDIRECT_PORT);
    return 1;
}

static inline int from_sidecar(struct bpf_sock_addr *ctx, __u32 curr_pod_ip)
{
...
{
    __u32 curr_pod_ip = get_curr_pod_ip(ctx);
    if (curr_pod_ip == 0) {
    ...
    if (uid != SIDECAR_USER_ID) {
        ...
        if (is_port_listen_current_ns(ctx, 0, OUT_REDIRECT_PORT)) {
            return from_app(ctx, curr_pod_ip);
        }
        __u32 ip = ctx->user_ip4;
        struct pod_config *pod = bpf_map_lookup_elem(&local_pod_ips, &ip);
        if (pod) {
            return to_app(ctx, pod);
        }
        // bypass normal traffic.
        // we only deal pod's traffic managed by istio.
        return 1;
    }
    return from_sidecar(ctx, curr_pod_ip);
}
